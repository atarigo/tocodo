...
Step 4 — 護甲穿透
  有效護甲 = 護甲值總和 × (1 − armorPenetration)
  armorPenetration 來自 modifier 收集（武器 + 技能加總，上限 1）

Step 5 — 破防判定
  incoming > 有效護甲         → 破防，進入 Step 6
  incoming > 有效護甲 ÷ 3    → 未破防，強制傷害 1，跳到 Step 8
  incoming ≤ 有效護甲 ÷ 3    → 完全擋下，傷害 0，跳到 Step 8

Step 6 — 護甲減算
  incoming = incoming − 有效護甲
...
Step 9 — 最終傷害
  ...
  （未破防強制 1 或完全擋下 0 的情況在 Step 5 已處理）
```
...
**穿透 (armorPenetration)**：數值 0~1，代表忽略多少比例的護甲。0 = 沒有穿透，1 = 完全穿透（等同舊的 pierce: true）。穿透值來自 modifier 系統，武器和技能可以各自貢獻、加總疊加。

傷害管線中護甲步驟改為：
```
有效護甲 = 護甲值 × (1 - armorPenetration)
```
`armorPenetration ≥ 1` 時護甲歸零（必定破防），仍吃減傷率和招架/格檔折減。
...
角色可以在主手和副手各裝一把武器。兩把武器各自獨立運作：

- 各自有獨立的攻速冷卻，依各自的武器間隔和角色敏捷計算
- 各自獨立出手，走同一套攻擊表 → 傷害管線 → 效果觸發
- 副手武器的招架率、格檔率各自貢獻到角色的防禦計算
- 雙手武器（帶 `two_handed` tag）佔據主手和副手，不能雙持

技能出手時使用近戰手（有近戰武器的那隻手），如果沒有近戰手就使用主手。

---

## 投射物系統

遠程武器（弓、槍械）攻擊時產生投射物飛向目標。投射物有飛行速度和碰撞半徑，命中目標後進入傷害管線。
...
投射物的飛行距離等於武器的攻擊距離。超過距離或命中目標後消失。

---

## AI 行為系統

所有非玩家的戰鬥單位（敵人、NPC）由 AI 控制，有以下行為狀態：

- **警戒（guard）**：待在原位不動，等待玩家進入偵測範圍
- **戰鬥（combat）**：追擊目標、攻擊、使用技能
- **歸位（returning）**：目標離開追擊距離後，放棄追擊回到原位

偵測距離和追擊距離來自敵人的資料定義（alertRange、leashRange）。

仇恨系統決定攻擊優先順序：每個敵人維護一張仇恨表，記錄對每個目標的累積仇恨值（來自傷害和治療）。戰鬥中優先攻擊仇恨最高的目標。
